...
A known-plaintext attack helper for recovering ZipCrypto internal keys from zip archives whose entries are expected to contain XML content. Built around [bkcrack](https://github.com/kimci86/bkcrack) (Biham-Kocher attack implementation). Designed for use during authorised penetration testing engagements where ZipCrypto-encrypted archives are in scope.
...
1. Calls `bkcrack -D` to decrypt the entire archive to a new zip file using the recovered internal key triple.
2. Extracts every file from the decrypted archive using Python's `zipfile` module.
3. SHA-256 hashes the source archive, the decrypted archive, and every extracted file.
4. Writes `manifest.json` to the output directory with all of the above plus method, timing, and entry provenance.
...
1. Examine the archive listing (`bkcrack -L TARGET.zip`) to identify entry names and sizes. Note which entries are smallest — smaller ciphertexts shrink the keyspace for wrong guesses.
2. Obtain a reference plaintext from the same application and version that produced the archive. Even a structurally identical file (same software, same document type, same export settings) is usually sufficient.
3. Run single-entry mode with `--reference` against the most predictable entry (relationship files and properties documents are often more templated than body content).
4. If the content type is genuinely unknown, `--force-declaration-guess` can be added to `--all` as a last resort. Expect it to have a low success rate against realistic Deflate-compressed content.
